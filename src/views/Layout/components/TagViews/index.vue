  ...
  >
    <el-scrollbar>
      <div class="tags animate__animated animate__fadeInDown animate__faster">
        <template v-for="(item, i) in settingStore.tagViewArr" :key="item.title">
          <el-tag
            :closable="settingStore.tagViewArr.length !== 1 && i !== 0"
...
watch(
  ...
  (newVal) => {
    ...
    let isInclude = settingStore.tagViewArr.find((item: routeObj) => {
      return item.path === rouiteObj.path
    })
    if (isInclude) return
    settingStore.tagViewArr.push(rouiteObj)
  },
  { immediate: true },
)
...
const closeTag = (i: number, path: string) => {
  if (path === route.path) {
    settingStore.tagViewArr.splice(i, 1)
    router.push(settingStore.tagViewArr[settingStore.tagViewArr.length - 1].path)
  } else {
    settingStore.tagViewArr.splice(i, 1)
  }
}
